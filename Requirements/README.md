...
- **Classification fallback (REQ-01)**: single combined scan flow (no mode picker); on ambiguous/low-confidence classification, the app does not ask the elderly user anything — the scan is flagged pending caretaker review (same `SCAN_ARTIFACT` mechanism as REQ-05/REQ-07) and resolved from the Web UI, consistent with REQ-17's rule that the Android app never asks anyone to make a data decision.
