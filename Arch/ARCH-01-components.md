...
- **Review queue** ([REQ-17](../Requirements/REQ-17-caretaker-review-and-override.md)): every scan the backend couldn't confidently process shows up here with its original image, for the caretaker to type in the correct data — this is the only place structured medicine data is ever manually entered.
...
### API layer
- REST endpoints grouped by requirement domain: `/scan` (classification, REQ-01), `/medicines`, `/prescriptions`, `/bills`, `/reminders`, `/escalation-contacts`, and (Phase 3) `/chat`.
- Thin — validates requests, calls the service layer, shapes responses. No business logic here.

### Service layer
- Where REQ-01–REQ-17 actually live: scan classification, chemical-equivalence matching (REQ-02), dosage source fallback chain including the caretaker-override tier (REQ-04), prescription/bill extraction fallback chains ending in pending-review rather than manual entry (REQ-05/REQ-07), refill math (REQ-08), interaction/expiry checks (REQ-12/REQ-14), escalation rules (REQ-13), caretaker review/override handling (REQ-17).
...
- Thin wrappers around external services: OCR, translation, TTS (REQ-03, Phase 1), and (Phase 3) the AI chat model (REQ-09, Sarvam AI prioritized).
